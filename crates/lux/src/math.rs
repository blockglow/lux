...
impl<T: Number + Negate> Vector<3, T> {
    pub fn dot(self, rhs: Self) -> T {
        self.0[0]
            ...
            .add(self.0[2].mul(rhs.0[2]))
    }
    pub fn cross(self, rhs: Self) -> Self {
        Self([
            self.0[1]
                .mul(rhs.0[2])
                .add(rhs.0[1].mul(self.0[2]).negate()),
            self.0[2]
                .mul(rhs.0[0])
                .add(rhs.0[2].mul(self.0[0]).negate()),
            self.0[0]
                .mul(rhs.0[1])
                .add(rhs.0[0].mul(self.0[1]).negate()),
        ])
    }
}
...
impl<const N: usize, T: Number + Default + Copy> core::ops::MulAssign for Matrix<N, N, T>
where
    [T; N * N]: MatrixMath<N, N>,
{
    fn mul_assign(&mut self, rhs: Self) {
        *self = *self * rhs;
    }
...
impl<const N: usize, const M: usize, T: Default + Copy> Matrix<N, M, T>
where
    [T; N * M]: MatrixMath<N, M>,
{
...
impl<const N: usize, const M: usize, T: Number + Copy> Default for Matrix<N, M, T>
where
    [T; N * M]: MatrixMath<N, M>,
{
    fn default() -> Self {
...
impl Quaternion {
    ...
    pub fn normalize(self) -> Option<Self> {
        ...
        let norm = unsafe { (self.real * self.real + self.imaginary.magnitude()).sqrt() };
...
pub struct Translation<T: Copy = f32>(Vector<3, T>);

#[derive(Default, Clone, Copy)]
pub struct Scale<T: Copy = f32>(Vector<3, T>);
